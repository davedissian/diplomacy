...
Voronoi::Voronoi(vector<vec2> &points,
    ...
    borderMinX(minX), borderMaxX(maxX), borderMinY(minY), borderMaxY(maxY)
{
    ...
    int i = 0;
    for (auto &s : sites) {
        float x = s.coord.x = points[i].x;
        float y = s.coord.y = points[i].y;
        i++;
...
void Voronoi::clip_line(VEdge *e) {
    ...
    if (!(x1 == x2 && y1==y2)) {
        e->reg[0]->edges.push_back(vec4(x1, y1, x2, y2));
        e->reg[1]->edges.push_back(vec4(x1, y1, x2, y2));
    }
}
